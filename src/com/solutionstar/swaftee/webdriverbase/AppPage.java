...
public class AppPage extends TestListenerAdapter {
	...
	public String getVisibleTextOfElement(WebElement elem) {
		String visibleText = (String) getJavaScriptExecutor().executeScript("var clone = $(arguments[0]).clone();" + "clone.appendTo('body').find(':hidden').remove();" + "var text = clone.text();" + "clone.remove(); return text;", elem);
	...
	public boolean isLinkPresentStartingWith(String link) {
		String locator = "//a[starts-with(text(),'" + link + "')]";
		return isElementPresent(By.xpath(locator));
	}
	...
	public void clickOnLinkStartingWithText(String linkText) {
		By locator = By.xpath("//a[starts-with(text(),'" + linkText + "')]");
		if (isElementPresent(locator)) {
			driver.findElement(locator).click();
		}
	}
	...
	public void waitForElementToDisappearSimpleWait(String xpath, int timeOutInSeconds) {
		try {
			...
			logger.error("No such element exception occured");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void waitForAJaxCompletion() {
		try {
			ExpectedCondition<Boolean> isLoadingFalse = new ExpectedCondition<Boolean>() {
				...
				public Boolean apply(WebDriver driver) {
					String ajaxCount = (String) ((JavascriptExecutor) driver).executeScript("return '' + XMLHttpRequest.prototype.ajaxCount");
			...
			Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(WebDriverConstants.WAIT_ONE_MIN, TimeUnit.SECONDS).pollingEvery(500, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class);
			wait.until(isLoadingFalse);
		} catch (Exception e) {
	...
	public void monkeyPatch() {
		 ...
		 * executing (function(xhr) { xhr.ajaxCount = 0; function
		 * incrementAjaxCount() { xhr.ajaxCount++; console.log('Ajax count when
		 * triggering ajax send: ' + xhr.ajaxCount); } function
		 * decrementAjaxCount() { if(xhr.ajaxCount > 0) xhr.ajaxCount--;
		 * console.log('Ajax count when resolving ajax send: ' + xhr.ajaxCount);
		 * } var send = xhr.send; xhr.send = function(data) {
		 * this.addEventListener('readystatechange', function() { if(this !=
		 * null && this.readyState == XMLHttpRequest.DONE) {
		 * decrementAjaxCount(); } }, false); incrementAjaxCount(); return
		 * send.apply(this, arguments); }; var abort = xhr.abort; xhr.abort =
		 * function(data) { decrementAjaxCount(); return abort.apply(this,
		 * arguments); }; return xhr; })(XMLHttpRequest.prototype);
		 */
		String ajaxCount = (String) ((JavascriptExecutor) driver).executeScript("return '' + XMLHttpRequest.prototype.ajaxCount");
		if (ajaxCount != null && ajaxCount.equals("undefined")) {
			getJavaScriptExecutor().executeScript("!function(t){function n(){t.ajaxCount++,console.log(\"Ajax count when triggering ajax send: \"+t.ajaxCount)}function a(){t.ajaxCount>0&&t.ajaxCount--,console.log(\"Ajax count when resolving ajax send: \"+t.ajaxCount)}t.ajaxCount=0;var e=t.send;t.send=function(t){return this.addEventListener(\"readystatechange\",function(){null!=this&&this.readyState==XMLHttpRequest.DONE&&a()},!1),n(),e.apply(this,arguments)};var o=t.abort;return t.abort=function(t){return a(),o.apply(this,arguments)},t}(XMLHttpRequest.prototype);");
		}
	}
	...
	public void assertTitle(String s) {
		Assert.assertEquals(s, this.driver.getTitle(), "Expected HTML title '" + s + "' but got '" + this.driver.getTitle() + "'.");
	}

	public void assertObjectExists(WebElement element) {
		Assert.assertTrue((element != null), "Expected object not found in page. Locator: " + element.toString());
	}
	...
	public boolean hasEditableFields(WebElement element) {
		boolean flag = (Boolean) (getJavaScriptExecutor().executeScript("var inputElements = arguments[0].getElementsByTagName('input');" + "	var textareaElements = arguments[0].getElementsByTagName('textarea');" + "	var selectElements = arguments[0].getElementsByTagName('select');" + "	var flag = false;" + "	for(var index = 0; !flag && index < inputElements.length; index++) {" + "		if(inputElements[index].type == 'hidden') {" + "			continue;" + "		}" + "		if(!inputElements[index].disabled) {" + "			flag = true;" + "		}" + "	}" + "	for(var index = 0; !flag && index < textareaElements.length; index++) {" + "		if(!textareaElements[index].disabled) {" + "			flag = true;" + "		}" + "	}" + "	for(var index = 0; !flag && index < selectElements.length; index++) {" + "		if(!selectElements[index].disabled) {" + "			flag = true;" + "		}" + "	}" + "	return flag;", element));
		return flag;
	}
