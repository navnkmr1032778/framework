 ...
 * touch file to mark a starting point, after which the files got created needs
 * to be copied to the destination. Currently this wrapper is created to handle
 * only csv files
 * 
 * @author Sgoutham
 *
...
public class SFTPWrapper {
	...
	private void logExitStatus(String command, int exitStatus) throws Exception {
		if (exitStatus < 0) {
			...
			throw new Exception("[User Exception] SFTP command '" + command
					+ "' execution done, but with error! Error code: " + exitStatus);
		} else {
	...
	public void createTarFile() throws Exception {
		execCommand("cd " + baseDirectory + ";tar -cf " + touchFilename + ".tar --newer ./" + touchFilename + " *.csv",
				true);
