...
public class ImageComparisonUtils implements ImageComparison {
	...
	public Map<String, Map<String, List<String>>> getAllCompareFilesByMethodNameAsMap() {
		...
		for (int instance = 0; instance < compareFoldersWithClassNames.length; instance++) {
			...
			for (int methods = 0; methods < compareFoldersWithMethodNames.length; methods++) {
				String comparePathWithMethodName = comparePathWithClassName + "/"
						+ compareFoldersWithMethodNames[methods];
	...
	public void addToCompareImagesListByFile(String compareFile, String baseFile, String storeTo,
			String baseMethodFolderName) {
		// String result;
	...
	public void addToCompareImagesListByFolderName(List<String> compareFiles, List<String> baseFiles,
			Map<String, String> filePath) {
		String storePath = WebDriverConstants.PATH_TO_BROWSER_SCREENSHOT_COMPARE_RESULT + "/"
				+ filePath.get("compareClassFolder") + "/" + filePath.get("deviceFolder") + "/"
				+ filePath.get("baseMethodFolder");
		File storeFile = new File(storePath);
		if (!storeFile.exists()) {
			storeFile.mkdirs();
		}
		String comparePath = currentDirLocation + "/" + filePath.get("compareClassFolder") + "/"
				+ filePath.get("deviceFolder") + "/" + filePath.get("baseMethodFolder");
		...
		while (compareFiles.size() > 0 && baseFiles.size() > 0) {
			...
			if (baseFiles.contains(compImage)) {
				addToCompareImagesListByFile(comparePath + "/" + compImage, basePath + "/" + compImage,
						storePath + "/" + compImage, basePath);
				...
				compImage = (compImage.substring(0, delStartIndex)
						+ compImage.substring(delEndIndex, compImage.length()));
				if (baseFiles.contains(compImage)) {
					addToCompareImagesListByFile(comparePath + "/" + compActualImage, basePath + "/" + compImage,
							storePath + "/" + compActualImage, basePath);
					compareFiles.remove(0);
					baseFiles.remove(compImage);
				} else {
	...
	public void getImageCompareList() {
		...
		while (compareClassIter.hasNext()) {
			...
			Map<String, Map<String, List<String>>> compareClassVal = (Map<String, Map<String, List<String>>>) compareClassEntry
					.getValue();
			...
			while (baseDeviceIter.hasNext()) {
				...
				Iterator baseMethodIter = (Iterator) ((Map<String, List<String>>) (baseDeviceEntry.getValue()))
						.entrySet().iterator();
				while (baseMethodIter.hasNext()) {
					...
					addToCompareImagesListByFolderName(compareClassVal.get(baseDeviceKey).get(baseMethodKey),
							baseImages, filePath);
				}
			}
			// while()
	...
	public void compareAllImages() {
		/*
		 * try { //downloadImageMagick(); } catch (MalformedURLException e) {
	...
	public String[] getListOfSubDirectories(String path, String folderName) {
		...
		String[] files = file.list(new FilenameFilter() {
			@Override
			public boolean accept(File current, String name) {
				return (new File(current, name).isDirectory() && name.split("_").length == 2
	...
	public void generateHTMLReport(Map<String, Map<String, List<String>>> resultMap) {
		...
		String start = "<!DOCTYPE html>" + "<html lang=\"en\">" + "<head>" + "  <title>Bootstrap Example</title>"
				+ "<meta charset=\"utf-8\">"
				+ "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
				+ "<link rel=\"stylesheet\" href=\"http://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css\">"
				+ "<script src=\"https://ajax.googleapis.com/ajax/libs/jquery/1.12.0/jquery.min.js\"></script>"
				+ "<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"" + swafteePath
				+ "/swaftee/resources/ImgCmpCss.css\" />"
				+ "<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"http://cdnjs.cloudflare.com/ajax/libs/fancybox/1.3.4/jquery.fancybox-1.3.4.css\" />"
				+ "</head>" + "<body>"
				+ "<script type=\"text/javascript\" src=\"http://code.jquery.com/jquery-1.11.0.min.js\"></script>"
				+ "<script type=\"text/javascript\" src=\"http://code.jquery.com/jquery-migrate-1.2.1.min.js\"></script>"
				+ "<script type=\"text/javascript\" src=\"http://cdnjs.cloudflare.com/ajax/libs/fancybox/1.3.4/jquery.fancybox-1.3.4.pack.min.js\"></script>"
				+ "<script type=\"text/javascript\" src=\"" + swafteePath
				+ "/swaftee/resources/ImgCmpScript.js\"></script>";
		output1.append(start);
		output1.append("<b>Image Comparison Test Status: " + className + "</b>");
		/*
		 * String
		 * style="<style> img:hover { position:relative; top:-25px; left:-35px; width:1000px; height:auto; display:block; z-index:999; </style>"
		...
		while (i <= compareFilesMap.size()) {
			output1.append("<th>Compare_Folder_" + i + "<div><input type='radio' name='all' value='all" + i
					+ "' data-toggle='unchecked'></div></th>");
			++i;
		}
		...
		while (entries.hasNext()) {
			...
			while (entries2.hasNext()) {
				...
				output1.append("<tr><td><img src=\"" + key2
						+ "\"  class=\"fancybox\" height=\"200\" width=\"200\" title=\"" + key2 + "\"/></td>");
				...
				for (String value : result2) {
					...
					output1.append("<td><img src=\"" + res[1] + "\"  class=\"fancybox\" height=\"200\" width=\"200\" "
							+ borderStyle + " title=\"" + actual + "\"/>" + "<div><input type='radio' name='" + key2
							+ "' value='" + actual + "' data-toggle='unchecked'>" + res[2] + "</div></td>");
				}
				output1.append("</tr>");
			}
		}
