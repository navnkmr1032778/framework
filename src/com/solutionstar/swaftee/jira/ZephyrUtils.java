...
import com.solutionstar.swaftee.beans.*;
...
public class ZephyrUtils {
	...
	/**
	 * Initializes the zephyr utils by setting the jira server, auth token from the
	 * system parameter values. The system parameters used are:
	 * <ul>
	 * <li>jiraServer - The URL of jira server</li>
	 * <li>jiraAuth - The auth token, the username and password of jira encoded
	 * using base 64</li>
	 * </ul>
	 * 
	...
	/**
	 * This method is used to get the cycle details. The following hashmap will be
	 * returned
	 * 
	 * <pre>
	 * {
	...
	/**
	 * This method is used to add the test cases which are not as part of the
	 * current test cycle
	 * 
	 * @param testCases The array of test case ids to be added to the cycle
	 * @return HashMap of the keys and their execution ids, which has to be used to
	 *         mark them pass/fail
	 */
	public static HashMap<String, String> createNewTestExecution(String[] testCases) {
		...
		/*
		 * Add test to cycle - request will return appropriate message if test is
		 * already added to the cycle
		 */
	...
	/**
	 * Takes the latest execution details for the given test cycle id, this is used
	 * initially to get the test cases in the test cycle
	 * 
	...
	public static void bulkUpdateStatus(Collection<String> executionIds, String status) {
		...
		logger.info("JIRA status that needs to be set : " + status + " - list of execution id's : "
				+ Arrays.toString(executionIds.toArray()));
	...
	/**
	 * This method is to reset i.e. set execution status as unexecuted for all the
	 * test cases in the test cycle
	 */
	 ...
	 * 
	 * @return List of HahMap which indicating the execution status of the test
	 *         cases
	 */
	public static List<HashMap<String, String>> getExecutionStatusFromCycle() {
		...
		for (LinkedTreeMap<String, Object> execution : executions) {
			...
			if ((mode != null && mode.equals("generate-report"))
					|| testCaseStatus.containsKey(execution.get("issueKey"))) {
	...
	public static void updateExecutionStatusInJIRA() {
		try {
			...
			HashMap<String, String> testCaseExecutionMap = getExecutionIdFromTestCycle();
			logger.info(
					"Updating Test Cycle - " + cycleId + " for the Test Cases - " + testCaseStatus.keySet().toString());
	...
	/**
	 * file will be saved in "resources/testCycleReport" folder in project. Create a
	 * folder in resource folder as "testCycleReport"
	 */
	public static void extractCSV(String versionId, String projectId, String auth, String testCycleId)
			throws IOException {
		...
		Response response = client.target(ZephyrConstants.TESTCYCLE_URL + testCycleId + "/export?versionId=" + versionId
				+ "&projectId=" + projectId).request().header(HttpHeaders.AUTHORIZATION, authInfo).get();
